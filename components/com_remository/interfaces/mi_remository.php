...
class mi_remository {
	...
	public function __construct () {
		...
		if (!defined('_JLEGACY') AND method_exists('mosMainFrame', 'getInstance')) $mainframe = mosMainFrame::getInstance();
		else global $mainframe;
		// Load the standard Remository CMS interface layer
		$this->absolute_path = $mainframe->getCfg('absolute_path');
		require_once($this->absolute_path.'/components/com_remository/remository.interface.php');
		$this->interface = remositoryInterface::getInstance();
		// Obtain the standard database object via the interface layer
...
class remository_restriction extends mosDBTable {
	...
	private $active				= null;
	/** @var int */
	private $granted_downloads	= null;
	/** @var int */
	private $unlimited_downloads	= null;
	/** @var text */
	private $used_downloads		= null;
	/** @var text */
	private $params				= null;
