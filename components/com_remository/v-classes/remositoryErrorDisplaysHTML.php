...
class remositoryErrorDisplaysHTML extends remositoryUserHTML {
	
	function noaccess ($fileorcontainer) {
		if ($fileorcontainer instanceof remositoryFile OR $fileorcontainer instanceof remositoryContainer) {
			...
			if ($container instanceof remositoryContainer ) $this->pathwayHTML($container);
		}
		echo <<<NO_ACCESS
...
UPLOAD_LIMIT;
	...
	public function noFileDiagnostics ($user) {
		...
		$filename = empty($_FILES['userfile']['name']) ? 'No file name provided' : $_FILES['userfile']['name'];
		$filesize = empty($_FILES['userfile']['size']) ? 'No file size provided' : $_FILES['userfile']['size'];
