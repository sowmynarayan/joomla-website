...
COMMENTS;
	...
	private function showLocaliseLink ($file) {
		...
		if (0 === strpos($file->url,$live_site) AND file_exists($oldpath)) return <<<LOCALISE_LINK
			...
			<p>
				<a href="$admin_site/index2.php?option=com_remository&act=files&task=localise&cfid=$file->id&containerid=$file->containerid">
					$linktext
				</a>
			</p>
...
MAIN_DIV;
		...
		echo <<<END_PAGE
				...
				<input type="hidden" name="orphanpath" value="$orphanpath" />
