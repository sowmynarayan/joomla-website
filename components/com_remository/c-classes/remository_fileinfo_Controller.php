...
class remository_fileinfo_Controller extends remositoryUserControllers {
	...
	public function fileinfo ($func) {
	    ...
	    if ((($file->licenseagree AND $file->license) OR trim($this->repository->Default_Licence)) AND ($func == 'startdown' OR ($func == 'finishdown' AND remositoryRepository::getParam($_POST, 'agreecheck', 'off') != 'on'))) {
		...
		// $database->setQuery("SELECT id FROM #__downloads_files WHERE repnum = $this->repnum AND (submitdate > '$file->submitdate' OR ('$file->submitdate' = submitdate AND id < $file->id)) ORDER BY submitdate DESC, id DESC LIMIT 1");
		...
		// Change for multiple repositories
		// $database->setQuery("SELECT id FROM #__downloads_files WHERE repnum = $this->repnum AND (submitdate < '$file->submitdate' OR ('$file->submitdate' = submitdate AND id > $file->id)) ORDER BY submitdate DESC, id LIMIT 1");
