...
class remositoryAdminContainers extends remositoryAdminControllers {
	private static $actions = array('upload', 'download', 'edit', 'selfApprove');

	public function remositoryAdminContainers (&$admin) {
...
HIDDEN_PARENT;
	...
	public function addTask () {
		...
		foreach (self::$actions as $action) $selector[$action] = $this->getRoleSelect(null, $action);
	...
	public function editTask () {
		...
		// Create and activate a View object
		$view = $this->admin->newHTMLClassCheck ('editContainersHTML', $this, 0, $clist);
		foreach (self::$actions as $action) $selector[$action] = $this->getRoleSelect($container, $action);
		$view->view($container, $selector, count($subsinfo));
	}
	...
	private function commonSave () {
	    ...
	    $container->moveFilesAsNecessary ($inheritpath);
	...
	public function deleteTask (){
		...
		foreach ($this->admin->cfid as $id) {
			...
			foreach (self::$actions as $action) $this->dropPermissions($authoriser, $action, $container, false);
			$this->parentid = $container->parentid;
			$container->deleteAll();
		}
